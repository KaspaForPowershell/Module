...
----------------------------------------------------------------- */
        ...
        {
            async Task<(decimal, ErrorRecord?)> processLogic(CancellationToken cancellation_token) { return await DoProcessLogicAsync(this._httpClient!, cancellation_token); }
        ...
        protected override void ProcessRecord()
        {
            ...
            {
                var (responce, err) = DoProcessLogicAsync(this._httpClient!, stoppingToken).GetAwaiter().GetResult();
                if (err is not null)
                {
                    WriteError(err);
                    return;
...
----------------------------------------------------------------- */
        ...
        private async Task<(decimal, ErrorRecord?)> DoProcessLogicAsync(HttpClient http_client, CancellationToken cancellation_token)
        {
            try
            {
